...
class UGATIT(object) :
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            print(
                f"step:[{step}/{self.iteration}],G_loss:{Generator_loss},D_loss:{Discriminator_loss},G_lr:{self.G_optim.param_groups[0]['lr']},"
                f"D_lr:{self.D_optim.param_groups[0]['lr']},time:{change_time(time.time() - start_time)}")
    ...
    def load(self):
        params = torch.load(os.path.join(self.result_dir,self.dataset,'model','selfie2animemodel.pt'))
